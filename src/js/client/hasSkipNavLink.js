...
(function () {
    ...
    fluid.registerNamespace("gpii.ptd.hasSkipNavLink");

    // TODO:  This pattern is used here and in `toggle.js`.  Harden it up and make a common grade.
    gpii.ptd.hasSkipNavLink.handleKeyPress = function (that, event) {
        if (event) {
            var handled = false;
    ...
    gpii.ptd.hasSkipNavLink.focusOnMain = function (that, event) {
        if (event) {
    ...
    fluid.defaults("gpii.ptd.hasSkipNavLink", {
        selectors: {
        ...
        invokers: {
            handleKeyPress: {
                funcName: "gpii.ptd.hasSkipNavLink.handleKeyPress",
                args:     ["{that}", "{arguments}.0"]
            },
            focusOnMain: {
                funcName: "gpii.ptd.hasSkipNavLink.focusOnMain",
                args:     ["{that}", "{arguments}.0"]
            }
        },
        listeners: {
    ...
    // A shim to make `gpii.ptd.hasSkipNavLink` work for plain old templates (non `templateAware`).
    fluid.defaults("gpii.ptd.hasSkipNavLink.nonTemplateAware", {
        gradeNames: ["gpii.ptd.hasSkipNavLink"],
        events: {
            onMarkupRendered: {
                events: {
                    onCreate: "onCreate"
