// TODO:  Agree on where this should live and move it from both the PTD and UL to that place.  Also, add tests.
//
...
(function () {
    ...
    fluid.registerNamespace("gpii.ptd.toggle");

    gpii.ptd.toggle.performToggle = function (that, event) {
        if (event) {
        ...
        that.events.onToggle.fire(that);
    };

    gpii.ptd.toggle.filterKeyPress = function (that, event) {
        if (event) {
            var handled = false;
    ...
    fluid.defaults("gpii.ptd.toggle", {
        ...
        invokers: {
            filterKeyPress: {
                funcName: "gpii.ptd.toggle.filterKeyPress",
                args:     ["{that}", "{arguments}.0"]
            },
            performToggle: {
                funcName: "gpii.ptd.toggle.performToggle",
                args:     ["{that}", "{arguments}.0"]
            }
        },
        events: {
        ...
        listeners: {
            // If I put multiple bindings in a single entry, only the first gets wired.
            // TODO:  Discuss compressing this with Antranig.  Ideally I would love to have a single line for all of this.
            "onRefresh.wireClick": {
                "this": "{that}.dom.toggle",
                method: "keydown",
                args:   "{that}.filterKeyPress"
            },
            "onRefresh.wireKeyPress": {
                "this": "{that}.dom.toggle",
                method: "click",
