...
require("../../../src/js/server/lib/sorting");
...
jqUnit.test("Test case-insensitive sorting (ascending)...", function () {
    var dataToSort = [ { a: "A" }, { a: "E" }, { a: "C" }, { a: "b" }, { a: "d" } ];
    var expectedResults = [ { a: "A" }, { a: "b" }, { a: "C" }, { a: "d" }, { a: "E" } ];

    gpii.ptd.api.lib.sorting.sort(dataToSort, ["a"]);

    jqUnit.assertDeepEq("The data should be sorted correctly...", expectedResults, dataToSort);
});

jqUnit.test("Test case-insensitive sorting (descending)...", function () {
    var dataToSort = [ { a: "A" }, { a: "E" }, { a: "C" }, { a: "b" }, { a: "d" } ];
    var expectedResults = [ { a: "E" }, { a: "d" }, { a: "C" }, { a: "b" }, { a: "A" } ];
...
jqUnit.test("Test sorting of null, undefined, and empty values...", function () {
    var dataToSort = [ { a: null, b: "c" }, { a: undefined, b: "b" }, { a: "", b: "a" }, { a: "a", b: "z"} ];
    var expectedResults = [ { a: "a", b: "z" }, { a: "", b: "a" }, { a: undefined, b: "b" }, { a: null, b: "c" }];

    gpii.ptd.api.lib.sorting.sort(dataToSort, ["a", "b"]);

    jqUnit.assertDeepEq("The data should be sorted correctly...", expectedResults, dataToSort);
});

jqUnit.test("Test isEmpty function...", function () {
    jqUnit.assertTrue("`null` should be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty(null));
    jqUnit.assertTrue("`undefined` should be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty(undefined));
    jqUnit.assertTrue("An empty string should be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty(""));
    jqUnit.assertTrue("A string that trims to nothing should be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty("    "));

    jqUnit.assertFalse("The number zero should not be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty(0));
    jqUnit.assertFalse("A non-empty string should not be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty("foobar "));
    jqUnit.assertFalse("A date should not be treated as empty...", gpii.ptd.api.lib.sorting.isEmpty(new Date()));
});
