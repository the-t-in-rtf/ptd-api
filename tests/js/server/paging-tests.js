...
require("../../../src/js/server/lib/paging");
...
gpii.ptd.api.tests.paging.runTests = function (that) {
    ...
    jqUnit.test("Test nested paging....", function () {
        var firstNestedResult  = that.pager.pageArray(that.pager.pageArray(that.options.testArray, {offset: 25, limit: 50 }), {offset: 20, limit: 5 });
        var secondNestedResult = that.pager.pageArray(that.pager.pageArray(that.options.testArray, {offset: 10, limit: 75 }),  {offset: 35,  limit: 5 });
    ...
    jqUnit.test("Test paging through non-array values...", function () {
        ...
        fluid.each(unacceptable, function (value, key) {
            jqUnit.expectFrameworkDiagnostic("Paging through " + key + " should throw an error...", function () { that.pager.pageArray(value); }, ["The paging component can't work with anything but an array."]);
        });
    });

    jqUnit.test("Test paging past the end of the record set...", function () {
        jqUnit.expectFrameworkDiagnostic(
            "Paging past the end of the result set should result in an error...",
            function () { that.pager.pageArray(that.options.testArray, {offset: that.options.testArray.length + 5}); },
            ["cannot page beyond the end"]
        );
    });
};

var testDataPath = path.resolve(__dirname, "../../data/pouchdb/records.json");
var testData = JSON.parse(fs.readFileSync(testDataPath, "utf8"));
